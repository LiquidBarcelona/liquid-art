class Controller {
    ...
    renderDiv(div, item) {
        $(div).append(
            `
                <div class="row">
                    <div class="card d-flex h-100" style="background: #f4f4f4;width: 38rem;">
                      
                      <div class="card-body d-flex flex-column align-items-start">
                        <h5 class="card-title">
                            <a href="${item.authorUrl}" target="_blank"> ${item.author}</a> -
                            <a href="${item.workOfArtWiki}" target="_blank">${item.title}</a>
                            (<a href="#year:${item.year}">${item.year}</a>)   &nbsp; &nbsp;
                            <a href="#title:${item.title}" class="bi bi-link float-lg-right"></a>
                        </h5>
                      </div>
